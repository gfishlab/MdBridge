...
pub fn get_branches(path: &str) -> Result<Vec<GitBranch>, String> {
    ...
    let output = run_git(
        &repo_root,
        &["branch", "--format=%(HEAD)%x1f%(refname:short)"],
    )?;
...
pub fn pull_repository(path: &str) -> Result<GitOperationResult, String> {
    let repo_root = repo_root_for_path(path)?;
    let output = run_git(&repo_root, &["pull", "--ff-only"])?;
    Ok(GitOperationResult {
        message: output.trim().into(),
    })
}
...
mod tests {
    use super::{
        parse_branch_output, parse_log_line, parse_status_output, GitBranch, GitCommit, GitStatus,
    };
    ...
    fn parses_branch_list_with_current_marker() {
        let branches = parse_branch_output(" \u{1f}draft\n*\u{1f}main\n \u{1f}release/v2\n");
